...
class TraitDoc(BaseDocument):
    ...
    def set_trait(self, trait):
        ...
        parent_section.append_rightside_anchor(Anchor('edit', href='hello.there.dude'))
        parent_section.append_rightside_anchor(Anchor('edit2', href='hello.there.dudee'))
        ...
        # SectionTitle doesn't understand href, it should probably be an Anchor
        # the href looks like a future call for implementing something anyway
        self.body.append(SectionTitle('Variables', href='foo.var.ick'))
        if len(self.trait.environ.keys()):
            env = TraitEnvTable(trait, self.trait.environ, bgcolor='MistyRose3')
            self.body.append(env)
        self.body.append(SectionTitle('Scripts'))
        slist = UnorderedList()
