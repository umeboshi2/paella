...
class Release(object):
    def __init__(self, source, arch='i386'):
        ...
        self.arch = arch
        self._zip_ = 'gz'
        if source.__class__.__name__ == 'RepositorySource':
            self.source = source
            ...
            self.parse_string(source[0])

    def __repr__(self):
        sums = ['%s: -> %s' %(s.name, s.md5sum) for s in self.sums.values()]
        return '\n'.join(sums)

    def __make_filelist__(self):
        paths = []
        for section in self.source.sections:
            paths.append(self.path(section))
            paths.append(self.path(section, release=True))
        return paths
    
    def _parse_md5sums(self, body):
        sums = [_Srcfile(*x.split()) for x in body.split('\n')[1:]]
        sumdict = dict([(x.name, x) for x in sums if x.name in self.__make_filelist__()])
        return sumdict
    ...
    def parse_string(self, release):
        self.fields = email.message_from_string(release.read())
        release.seek(0)
        self.sums = self._parse_md5sums(self.fields['md5sum'])
    ...
    def midpath(self, arch='i386'):
        if self.source.type == 'deb-src':
            return SRCPATH
        elif self.source.type == 'deb':
            return _binpath(arch)
        else:
...
class Debootstrap(object):
    def __init__(self):
        ...
        self.suite = 'woody'
        self.root = None
        self.mirror = 'file:/mirrors/debian'
        self.script = ''
